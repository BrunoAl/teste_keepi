...
class Table extends Component {
  ...
  renderReturnToPrevDir() {
    const getPrevDirNameFromPath = path => {
      // has parent dir
      if (path.split('/').length > 2) {
        // create an array of the folder and files of the path, then we pop the last file and folder from the array. At this point we create a new path with the remaining parts of the array, and lastly we pop the "/" from the front of out new path :)
        return this.state.repoContent[0].path.split('/').slice(0, -2).reduce((a, b) => `${a}/${b}`, '').substr(1)
      } else {
      ...
      return (
        <tr>
          <td>
            ...
            }<a href="#" onClick={() => this.listRepoFiles(this.state.repoUserName, this.state.repoName, getPrevDirNameFromPath(this.state.repoContent[0].path) )}>..</a>
            </p>
          </td>
        </tr>
      )
                ...
                return (
                  <tr key={content.sha+content.name}>
                    <td>
                      <p>{content.type === "dir" ?
                        // eslint-disable-next-line
                        <a href="#" onClick={() => this.listRepoFiles(this.state.repoUserName, this.state.repoName, content.path)}>{content.name}</a> :
                        content.name}
                      </p>
                    </td>
                  </tr>
